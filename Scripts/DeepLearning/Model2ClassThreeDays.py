    ...
    for i in range(0, len(input) - group_size):
        ...
        if max_week_volume > 0 or noVolume_prob:
            ...
            if (i < (len(input) - group_size - 1) * 0.8):
                x_train.append(np.append(input[i_pos:group_size + i_pos], label[i_pos - 1 - 3: i_pos -1 -3 + group_size], axis=1))
                y_train.append(label[i + group_size - 1])
            else:
                x_test.append(np.append(input[i_pos:group_size + i_pos], label[i_pos - 1 - 3: i_pos -1 -3 + group_size], axis=1))
...
def exec_model(path_output, inp_cols=[], label='',
               path_input='../../../OnlineData/CategoricalVolume_8cpFinalBinary.csv'):
    ...
    month_serie = list(map(lambda x: x.split('-')[1], np.array(df['TimOcurrencia'])))
    ...
    x_train = x_train.reshape(-1, group_size, len(input_transformed[0])+2)
    y_train = y_train.reshape(-1, len(label_transformed[0]))
    x_test = x_test.reshape(-1, group_size, len(input_transformed[0])+2)
